    ...
    try:
        with conn: # Transacción automática
            with conn.cursor() as cursor:
                ...
                cursor.execute(sql_dir_emp, ('Emplazamiento', direccion_data.get('tipo_via_id'), direccion_data.get('nombre_via'), direccion_data.get('numero_via'), direccion_data.get('piso_puerta'), direccion_data.get('codigo_postal'), direccion_data.get('localidad'), direccion_data.get('provincia')))
                direccion_emplazamiento_id = cursor.fetchone()['id']

                # Paso 2: Crear hospital y su dirección (si aplica)
                hospital_id = None
                ...
                # Paso 3: Crear la instalación con las nuevas columnas
                sql_instalacion = """
                    INSERT INTO instalaciones (
                        app_user_id, cliente_id, promotor_id, instalador_id, direccion_emplazamiento_id,
                        tipo_finca_id, panel_solar_id, inversor_id, bateria_id, distribuidora_id,
                        tipo_instalacion_id, tipo_cubierta_id, hospital_cercano_id, numero_pedido_presupuesto,
                        descripcion, numero_paneles, numero_inversores, numero_baterias, cups,
                        potencia_contratada_w, referencia_catastral, protector_sobretensiones,
...
def update_instalacion(conn, instalacion_id, app_user_id, data):
    """
    Actualiza una instalación existente y sus datos anidados (direcciones, hospital, tramos)
    de forma transaccional, segura y completa.
    """
    # Extraemos los datos anidados del JSON principal
    dir_emplaz_data = data.get('direccion_emplazamiento', {})
    hospital_data = data.get('hospital_cercano') # Usamos .get() que devuelve None si no existe
    tramos_data = data.get('tramos_cableado', [])
    ...
    try:
        with conn:  # Inicia la transacción. COMMIT o ROLLBACK es automático.
            with conn.cursor() as cursor:
                ...
                # Verificamos propiedad y obtenemos los IDs actuales para saber si debemos UPDATE o INSERT.
                cursor.execute(
                ...
                sql_update_instalacion = """
                    UPDATE instalaciones SET
                        ...
                        -- Nuevos campos
                        tipo_instalacion_id = %(tipo_instalacion_id)s,
                        tipo_cubierta_id = %(tipo_cubierta_id)s,
                        numero_pedido_presupuesto = %(numero_pedido_presupuesto)s,
                        hospital_cercano_id = %(hospital_cercano_id)s
